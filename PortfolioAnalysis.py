...
class PortFolAnalysis:
    ...
    def random_portfolios(self, num_portfolios=1000, risk_free_rate=0.0178):
        ...
        plt.scatter(params[0], params[1], c = params[2], cmap = "YlGnBu", marker = "o",s=10, alpha=0.9)
        plt.ylabel("Return")
        plt.xlabel("Volatility")
        plt.colorbar(label = "Sharpe ratio")
        #plt.plot( params[1][idx_max_retrn], params[0][idx_max_retrn], 'g')
        plt.scatter( params[0][idx_max_retrn],params[1][idx_max_retrn], marker = "*" ,s = 500, color = 'g', label = "Maximum Return")
        #plt.annotate("  Maximum Return",(params[0][idx_max_retrn], params[1][idx_max_retrn]))
        plt.scatter(params[0][idx_min_vol],params[1][idx_min_vol],  marker = "*" , color = 'r', s=500, label = "Minimum Volatility")
        #plt.annotate("  Minimum Volatility",( params[0][idx_min_vol], params[1][idx_min_vol]))
        plt.scatter(params[0][idx_max_sharpe], params[1][idx_max_sharpe], marker = "*" , color = 'y', s=500, label = "Maximum Sharpe Ratio")
    ...
    def Optimal_weights(self, req_return):
        ...
        print ("****************************************************************")
        print ("Expected return = {}%".format(round(100*ret.value,2)))
        print ("Expected risk    = {}%".format(round(100*risk.value**0.5,2)))
        optim_std_dev, optim_return = PortFolAnalysis.__annual_performance(self, x.value)
        print ("*****************************************************************")
        print ("Volatility and return of optimized portfolio are: ", optim_std_dev, " and ", optim_return)
        print ("Weights of optimal portfolio are: ", x.value)
